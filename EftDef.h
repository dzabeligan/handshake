...
typedef struct OriginalTransData {
  TransType transType;
  int sequenceNumber;
  char unixTime[32];
  char tid[16];
...
typedef struct EFTResponse {
  ...
  char balance[64];
  char available[64];
  char iccResponse[0x200];
  char authID[12];
  short dataLen;
  MiniStatementData miniStatementData[MINISTATEMENT_DATA_MAX];
} EFTResponse;

typedef struct HostDecision_t {
  int batchNumber;
  int sequenceNumber;
  char merchantId[16];
  char tid[16];
  ...
  Host host;
  IccData iccData;
  OriginalTransData originalTransData;
  Pinblock currentblock;
  Pinblock newblock;
  Pinblock confirmblock;
...
typedef struct Device {
  ...
  char ptadName[35];
  char oemName[35];
  char serviceSetIdentifier[33];  // ssid
  char closeBatchTime[22];
  short shouldIgnoreTerminalDecision;
  char amountSeparator[2];
  int amountDp;
  char amountDpSeparator[2];
  int currencyExp;
  char bankName[35];
  char bankId[25];
  int keyIndex;
  ...
  // Properties
  char tamsMerchantName[120];
  char tamsMerchantAddress[120];
  char rrn[21];
  char stampLabel[41];
  short accountSelection;
  char stampDutyThreshold[13];
  char stampDuty[13];
  char preConnect[5];
  char posSupport[45];
  char tid[17];
  ...
  char appName[35];
  char model[35];
  char supervisorPin[17];
  char supervisorYyyymmddhhmmss[15];
  char maintainerPin[17];
  char maintainerYyyymmddhhmmss[15];
  int currentBatchNumber;
  int currentSequenceNumber;
  char simId[25];
  char commsName[35];
  short shouldDisablePinblock;
  short shouldMapTid;
  short shouldAutoPrep;
  char platformFromTams[35];
  short shouldUseTpdu;
  char lastTransAt[35];
  char lastRrn[13];
  char simOperator[35];
  char notificationId[35];
  char netProfile[65];
  short noMiddlewareForNibss;
  unsigned char wifiSsid[65];
  int eftLifetime;
  char merchantPath[64];
  char name[35];
  char eftMenuName[45];
  short useShortDeclinedReceipt;
  short shouldRollbackAllApprovedEft;
  int libraryVersion;

  // Host
  // TODO Should be removed in library update
  char host[65];
  char port[7];
  short isSsl;
